...
## 🔧Technology Stack
- **IntelliJ** IDEA 2025 (Community Edition)
- **Java 21**
- **Spring Boot 3**
- **Spring REST API**
...
### AWS prerequisites
- Registered domain name ([AWS Guide](https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/domain-register.html))
- SSL Certificate
...
| **id**             | String     | Unique identifier generated by the backend repository, typically a numeric string. Should be omitted when creating a Task.                                                                                                                                                                  |
| **status**         | String     | Task status, which can be `"Initial"` or `"Deleted"` in this version. Can be omitted at creation and defaults to `"Initial"`.                                                                                                                                                               |
| **due**            | String     | Date and time in ISO‑8601 offset date‑time format, optionally including a time‑zone region.<br>Examples: <small>`2026-01-18T18:13:52.488Z`, `2026-04-18T00:13:52+01:00`, `2026-04-01T12:13:52+02:00[Europe/London]`</small><br>Can be omitted at creation and defaults to the current time. |
| **title**          | String     | Required non‑blank field of a Task.                                                                                                                                                                                                                                                         |
| **description**    | String     | Optional description of a Task.                                                                                                                                                                                                                                                             |
...
### Functional Testing
With Spring and a database repository, you typically have a Spring test version for Functional tests.
...
The app relies on enough logging in the underlying Spring and Eclipse Store frameworks when handling service requests.
Exceptions are wrapped into customised TaskExceptions,
which is intended to have more friendly information and not to reveal (for security concerns)
too much of the underlying framework.
...
The prerequisites instruct to have a working AWS Component Development toolKit.
...
where
* `infrastructure.jar` is the executable shadow jar with our CDK application
* `functions.jar`      is the shadow jar with our java compiled lambda code
* `your account`       is your AWS account number (see [below](#CDK-steps))
* `your region`        is something like eu-west-1
* `your domainname`    is your domain name (see [above](#AWS-prerequisites))
...
The main _gradle.build_ should contain a task to generate a shadowJar named _functions.jar_, which contains
...
2. Navigate to the _assets_ directory
3. Environment variable _CDK_DEFAULT_ACCOUNT_ should get set, automatically, during the session,
   in the background by CDK using AWS CLI to make a connection. It requires having
...
With the Spring annotations and persistence requirements for the Eclipse Store framework, the TDD approach
...
The AWS support of EclipseStore does allow re-entrant database access, but not
distributed locking, as I hoped. EclipseSore allows a lambda with just one instance.
That can be configured for lambdas. However, that defeats the role lambdas play, namely,
automatically increase/decrease the number of available instances depending on the number of requests.
Also, the SpringBoot application can only have one instance running, permanently,
to control and avoid issues with multiple access to the same database from different instances.
...
I created a DynamoDB solution design, where the database access is the bottleneck, so that I can demonstrate
a Serverless solution, and a migration strategy for a serverless solution in combination of DynamoDb.

AWS Lambda might be a technology preferred over SpringBoot. Lambdas are also event driven code
which is running on provisioned, scaled, patched AWS infrastructure. Lambdas cost are calculated
for the milliseconds of execution and the size of runtime framework, which is more cost-effective than SpringBoot.
Lambdas integrate with a Gateway API for security and DynamoDB as a distributed database,
i.e. allows multiple back-end instances using the database performant and reliable.

To get the AWS architectural diagram, I used Microsoft CoPilot to generate it from the question
_Give me an AWS architectural diagram of route53, api-gateway, five lambdas, and a dynamodb table. The lambdas in
the diagram are labelled respectively "Root", "Create", "Delete", "Retrieve" and "Update". All lambdas, except
the lambda labelled "Root", connect to the table. That image is shown above.
...
libraries that cause a not responding lambda. Those libraries load during invocation, causing a timeout
after 10 seconds. That made me splitting up the code in separate IDE/Gradle modules. By that, controlling
what is inside a shadowJar, is easier.
